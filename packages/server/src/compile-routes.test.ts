...
describe("compile routes", () => {
  ...
  it("compiles a blocker-free project and is deterministic across calls", async () => {
    ...
    expect(firstBody.metadata.versions).toEqual({ template: "1.11.0", compiler: "1.13.0", contract: "1.15.0" });
  ...
  it("compiles an ideation preview with relaxed readiness", async () => {
    ...
    expect(body.metadata.versions).toEqual({ template: "1.11.0", compiler: "1.13.0", contract: "1.15.0" });
  });
