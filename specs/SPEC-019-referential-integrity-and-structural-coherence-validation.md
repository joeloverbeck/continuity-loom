...
- `packages/core/src/records/registry.ts:21,53` and per-type `extractReferences` declarations (`entity.ts:57-72`, `causal-pressure.ts:157-218`, `knowledge.ts:92-111`, `cast-member.ts:152`) — a reference-extraction mechanism (`extractRecordReferences`, exported at `packages/core/src/index.ts:166`) already exists with per-type coverage but has **no validation/server consumer**.
...
The implemented 51-rule validation pipeline is comprehensive on presence/completeness (is required state filled in?) and covers a handful of contradictions, but the **identity layer** — whether the record ids that wire the generation brief and the selected records together actually point at real, selected, correctly-typed records — is almost entirely unvalidated:
...
Record-internal references reuse the dormant `extractRecordReferences` infrastructure rather than hand-written walkers; its per-type declarations are completed and drift-tested so a future recordId-typed schema field cannot silently escape validation. Structural-contradiction rules (onstage/offstage overlap, status-location contradiction, object holder incoherence, self-relationship) are plain enum/id comparisons.
...
- `current-location-reference-invalid`: a `current_location` value that matches the project index must be LOCATION-typed (else blocker) and selected (readiness-required lane → blocker). A value matching nothing in the index is treated as a prose scene-space label (sanctioned by contract §4) — no diagnostic.
...
- Complete `extractReferences` declarations for every recordId-typed payload field across all record types (audit: `SECRET.holders/non_holders_to_protect`, `OBJECT.owner/carried_by/current_location`, `RELATIONSHIP.from/to`, `AFFORDANCE.available_to`, `EVENT.participants/causes/effects` recordId branch, `FACT/EVENT.known_by` recordId branches, plus existing declarations). Add a drift test asserting every recordId-typed schema field is either extracted or explicitly exempted with a reason.
- `record-reference-dangling` (blocker): any extracted reference from a selected record whose target is absent from the project index.
...
- `docs/compiler-contract.md`: bump contract pin to `1.3.0` (with `packages/core/src/version.ts`); add the D2 severity lane table and new blocker/warning rows to §6; delete the POV must-not-block warning row; amend the §9 raw-id paragraph (raw-id fallback persists only in optional lanes and is always warning-surfaced; required lanes block before compilation); note the validation-only project record index.
...
5. **Index size.** The project record index grows with project size; it is ids and type strings only, and the existing `prompt-middle-salience-risk` warning keys off snapshot JSON size — confirm at implementation that the index is excluded from that size heuristic or the threshold is adjusted, so adding the index does not spuriously trip the warning.
