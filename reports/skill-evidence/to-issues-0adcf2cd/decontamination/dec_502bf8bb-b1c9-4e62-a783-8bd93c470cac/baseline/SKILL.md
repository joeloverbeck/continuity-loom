...
Before drafting, classify the source relationship for every proposed issue. Use **child mode** when the source tracker item is a parent whose scope the issue implements. Use **standalone-source mode** when the new issue is derived from a tracker item but is a predecessor, follow-on, or coordination issue rather than its child; name the exact relationship in durable wording such as `Blocks PRD #379`. Use **artifact-source mode** when the source is a durable local plan, spec, or report with no truthful tracker parent/source; name its exact repo-relative path, publication ref, source token, and relationship. Do not force an inverse, coordination, or artifact relationship into a `Parent` section.

In child mode, exact-read the parent's current state and labels and compare them with the canonical triage roles before Step 4. If any proposed child will be `ready-for-agent` while the parent is also `ready-for-agent`, the approval checkpoint must include an explicit parent-label transition that leaves the parent in a truthful non-AFK state. Do not invent an unlabeled or semantically false fallback. If the repository has no truthful non-AFK state for an open coordination parent, stop before publication and ask the maintainer to decide the tracker-state model. Never create an AFK-ready child family beside an AFK-ready parent.
...
When exit `2` reports `kind: sandbox-restricted`, rerun through the host's approved execution escalation when that mechanism is available. If escalation is unavailable or inappropriate, use the complete manual fallback below. Treat any other exit-2 kind as a helper/Git failure and preserve its structured error in the Step 4 evidence.
...
The mirror hazard is durable doctrine the source plan *contradicts* rather than one that is missing: a PRD or plan whose cited spec, ADR, or principle exists and is tracked, but whose proposed names, roles, or behavior conflict with it — a source may even claim non-contradiction while contradicting. Do not slice such a plan as written. Resolve the repository's authority order from its agent instructions and active authority registry first; use `docs/principles/README.md` only when that live guidance selects it or the file exists and governs the touched surface. Correct the source's assumptions per that resolved order, present the conflict and your proposed correction as an explicit decision in the Step 4 quiz — the resolution is the user's call — and encode the corrected version in the slices with the deviation named in each affected issue body. The correction lives in the child issues; never edit the parent.

When a slice's correctness hinges on a *derivation* a cited spec or ADR made — a choice it reached by analogy or judgment, not one the upstream authority dictates — confirm that derivation against the upstream named in the resolved authority order before encoding it, and flag it to the steward as a judgment call. Affirming non-contradiction with the top of that order is already owed, so a spec faithfully "respected" can still violate the higher authority if the spec itself drifted; if the derivation proves unsound, route the fix as a doctrine-correction (above) rather than propagating it into slices.
...
After the numbered list, include these eight global posture lines and self-check that none is missing:

- `Decision scan: <live or fallback source>; <hits and classification, or none>`
- `Source relationship: <child of #N, standalone source #N, or artifact source path/ref with exact relationship>`
- `Parent disposition: <current parent state/labels, approved transition and resulting labels, or N/A for source modes>`
...
- `Publication posture: <dependency order, label set, and child-ledger, standalone-source, or artifact-source posture>`
...
- In child mode, does the proposed parent disposition truthfully prevent the parent and children from remaining AFK-ready at the same time? If no canonical state fits, stop for a tracker-state decision instead of treating approval as authorization to improvise one.
- In child mode with multi-issue publication, should the optional parent child-map ledger comment be posted after creation, assuming tracker docs or house style allow it? In standalone-source and artifact-source modes, record the ledger posture as N/A.
...
For multi-issue breakdowns, end the checkpoint with an explicit publication sentence that states approval covers the eight posture lines as well as the numbered slices. In child mode: "If you approve this breakdown and its decision, tracker, parent-disposition, source/target, prerequisite, publication, artifact, and coverage postures, I will apply the approved parent-label transition, publish these N child issues in the dependency order shown with labels <labels>, and <post/skip> the parent child-map ledger as specified." Omit the transition clause when the parent is already in a truthful non-AFK state. In either source mode, use the same posture list with parent disposition N/A and say that the issues will carry the exact approved source relationship and no parent ledger; artifact-source mode must also name the durability proof. Adjust an open child-mode ledger clause before publication.
...
When the proposed breakdown is a single no-blocker issue, a compact approval question is enough, but it must still make approval cover the one-slice granularity and all eight posture lines, including parent disposition, no-prerequisite, artifact, coverage, ledger, and publication decisions. In child mode, default the ledger to `skipped — relationship rationale is complete in the issue body` when a separate child-map comment adds no information. In either source mode, parent disposition and the parent ledger are N/A because the issue body owns the exact source relationship. For example: "Does this one-slice/no-blocker breakdown and its eight postures feel right, and should I publish it as specified?"
...
1. Freeze the approved titles, dependencies, labels, source relationship, parent disposition, ledger posture, and story/checklist mappings.
...
5. In child mode, validate and post the approved parent ledger or apply its approved fallback; in either source mode, verify the approved source relationship in the issue body, plus artifact durability when applicable.
