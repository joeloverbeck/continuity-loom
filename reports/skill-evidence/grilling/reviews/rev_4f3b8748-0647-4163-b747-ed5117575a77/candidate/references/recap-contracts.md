...
A class can shift mid-session as facts surface. When it does, the recap owed is the **union** of the contracts for every class the run actually occupied. For a diagnostic-to-operational shift on the same subject, normalize overlapping labels before the final preflight: `Verdict` absorbs `Finding`; `Rejected/no-op alternatives` absorbs `Rejected operations`; and `Freshness/external research` absorbs both `External research` and `Freshness`. Do not also require the absorbed labels; require every other field from both classes. This normalization applies only to a genuine class shift, never to simultaneous independent items. Do not let the later class silently retire the earlier class's provenance.
...
rules above, then apply every matching scan below. For a class shift, apply the exact label
normalization above, then scan every occupied class for the remaining labels; for
