...
export const validateTddCloseoutBody = (body, options = {}) => {
  ...
  // The requirement is that the claim covers every row, not that it uses one exact bigram,
  // so any universal quantifier reaching the noun qualifies. Non-universal quantifiers
  // (`some rows`, `most rows`) and an unquantified `rows` still fail.
  const universalRowClaim = /\b(?:all|every|each)\b(?:\s+[\w-]+){0,3}\s+rows?\b/i;
  ...
  const validateAcceptanceAtomMapValue = (value) => {
    ...
    if (universalRowClaim.test(normalized) && /\batoms?\b.+\bproof surfaces?\b/i.test(normalized)) return "";
    if (/\ball criteria (?:are )?atomic\b.+\bproof surfaces?\b/i.test(normalized)) return "";
    return "must state that all rows (or every/each row) list authoritative atoms and proof surfaces, that all criteria are atomic with proof surfaces listed, or blocked because. Example: 'all rows in the acceptance audit below list authoritative atoms and proof surfaces'";
  };
  ...
  const validateAcceptanceSequenceMapValue = (value) => {
    ...
    if (/^<.*>$/.test(normalized)) return "is unresolved placeholder";
    if (/\bblocked\b.+\b(because|reason|unable|cannot)\b/i.test(normalized)) return "";
    if (universalRowClaim.test(normalized) && /\b(sequence|ordered proof|sequence N\/A)\b/i.test(normalized)) return "";
    if (/\ball criteria\b.+\bnot sequence-sensitive\b/i.test(normalized)) return "";
    return "must state that all rows (or every/each row) list ordered proof or justified sequence N/A, that all criteria are not sequence-sensitive, or blocked because. Example: 'all rows list ordered proof or a justified sequence N/A'";
  };

  // An unresolved authority state is a hard stop, but merely naming a conflict, a blocker,
  // or an unresolved question inside a resolved disposition is not one. Match the state,
  // not the vocabulary, so `aligned because ...; no conflict with ADR-0003` and a basis
  // that cites this project's blocker nouns stay admissible.
  const recordsUnresolvedAuthority = (normalized) =>
    /\bconflict\b\s*[-–—:]\s*blocked\b/i.test(normalized) ||
    /\b(?:unresolved|open|outstanding|unaddressed)\s+(?:authority\s+)?(?:conflict|question|amendment|exception)\b/i.test(
      normalized
    ) ||
    /\b(?:conflict|authority question)\b[^;.]{0,60}\b(?:remains?|is|are|still)\b[^;.]{0,24}\b(?:unresolved|open|blocked|not resolved)\b/i.test(
      normalized
    );

  const unresolvedAuthorityError =
    "records an unresolved authority conflict; resolve it before the first red command";

  const validateAuthorityDispositionValue = (value) => {
    const normalized = value.replace(/\s+/g, " ").trim();
    if (!normalized) return "is empty";
    if (/^<.*>$/.test(normalized)) return "is an unresolved placeholder";
    if (recordsUnresolvedAuthority(normalized)) return unresolvedAuthorityError;
    if (/^N\/A\b.+\bbecause\b.+/i.test(normalized)) return "";
    ...
    if (/\b(?:conflict|blocked|unresolved)\b/i.test(normalized)) return unresolvedAuthorityError;
  ...
  const identityRefreshUnresolved = unresolvedEvidenceToken(identityRefreshValue);
  if (identityRefreshUnresolved) {
    errors.push(`Evidence identity refresh is empty or unresolved: ${identityRefreshUnresolved}`);
  }
  ...
  const identityFieldUnresolved = [
    ["Current evidence identities", currentIdentities],
    ["Historical red identities retained", historicalRedIdentities],
    ["Superseded evidence identities", supersededIdentities],
    ["Superseded-token sweep", supersededSweep]
  ]
    .map(([label, value]) => [label, unresolvedEvidenceToken(value)])
    .filter(([, token]) => token);
  if (identityFieldUnresolved.length) {
    errors.push(
      `evidence identity fields are empty or unresolved: ${identityFieldUnresolved
  ...
  for (const { lineNumber, cells } of compactRows) {
    ...
    if (
      ...
      errors.push(
        /\bcoverage-only existing behavior\b/i.test(redCell)
          ? `compact TDD row ${lineNumber} red command/failure is not concrete: ${redCell}; a coverage-only existing-behavior row must justify the skip, as in \`coverage-only existing behavior; red-first N/A because behavior already existed and no production code changed\``
  ...
  for (const [index, cells] of reviewFixMapRows.entries()) {
    ...
    for (const [label, value] of [
      ...
      const unresolvedToken = unresolvedEvidenceToken(value);
      if (unresolvedToken) {
        errors.push(`TDD review-fix map row ${row} ${label} is empty or unresolved: ${unresolvedToken}`);
      }
    }
