...
const bodyWith = ({
  ...
  sequenceMap = "all rows list ordered proof or justified sequence N/A",
...
- Acceptance sequence map: ${sequenceMap}
...
const coverageOnlyBodyWith = (red) =>
  bodyWith().replace(
...
test("accepts identity citations that quote placeholder-like tokens", () => {
  for (const artifacts of [
    "`reports/pending-review-notes.md`",
    "golden prompt containing the `<audience_knowledge>` block"
  ]) {
    const body = bodyWith({
      current: `fixture paths none; browser sessions issue-1; packet paths/hashes proposal.txt abc123; active revisions run-2; artifacts ${artifacts}`
    });
...
test("names the unresolved token when an identity field is a placeholder", () => {
  const body = bodyWith({
    current:
      "fixture paths <paths>; browser sessions none; packet paths/hashes none; active revisions none; artifacts none"
  });
  ...
  assert.ok(
    validateTddCloseoutBody(body).some((error) =>
      error.includes("evidence identity fields are empty or unresolved: Current evidence identities <paths>")
    )
  );
});

test("accepts a resolved authority disposition that names a conflict or blocker", () => {
  for (const disposition of [
    "aligned because `docs/principles/FOUNDATIONS.md` §11 authorizes the blocker seam; no conflict with ADR-0003",
    "aligned because `docs/principles/FOUNDATIONS.md` §11 governs the blocked-demo inventory",
    "N/A because this documentation-only seam has no unresolved governing authority"
  ]) {
    assert.deepEqual(
      validateTddCloseoutBody(
        bodyWith({
          authorityRow: disposition,
          authorityPreflight: disposition,
          authorityGate: disposition
        })
      ),
      []
    );
...
test("still rejects an authority disposition whose conflict remains unresolved", () => {
  for (const disposition of [
    "conflict - blocked because FOUNDATIONS forbids the requested behavior",
    "aligned because ADR 0001 covers the seam, but the conflict with ADR-0003 is unresolved",
    "unresolved authority question about ADR-0003"
  ]) {
    assert.ok(
      validateTddCloseoutBody(
        bodyWith({
      ...
      ).some((error) => error.includes("unresolved authority conflict"))
    );
  }
});

test("accepts any universal quantifier in the acceptance atom and sequence maps", () => {
  for (const atomMap of [
    "all rows list authoritative atoms and proof surfaces",
    "all acceptance-audit rows list authoritative atoms and proof surfaces",
    "every row lists authoritative atoms and proof surfaces",
    "each compact row lists authoritative atoms and proof surfaces"
  ]) {
    assert.deepEqual(validateTddCloseoutBody(bodyWith({ atomMap })), []);
  }

  for (const sequenceMap of [
    "every row lists ordered proof or a justified sequence N/A",
    "all acceptance-audit rows list ordered proof or a justified sequence N/A"
  ]) {
    assert.deepEqual(validateTddCloseoutBody(bodyWith({ sequenceMap })), []);
  }
});

test("rejects a non-universal acceptance atom or sequence claim", () => {
  assert.ok(
    validateTddCloseoutBody(
      bodyWith({ atomMap: "some rows list authoritative atoms and proof surfaces" })
    ).some((error) => error.includes("Acceptance atom map"))
  );
  assert.ok(
    validateTddCloseoutBody(bodyWith({ atomMap: "rows list atoms" })).some((error) =>
      error.includes("Acceptance atom map")
    )
  );
  assert.ok(
    validateTddCloseoutBody(bodyWith({ sequenceMap: "most rows list ordered proof" })).some((error) =>
      error.includes("Acceptance sequence map")
    )
  );
});

test("accepts connective wording in a coverage-only existing-behavior red cell", () => {
  for (const red of [
    "coverage-only existing behavior; red-first N/A because behavior already existed and no production code changed",
    "coverage-only existing behavior; red-first N/A because the behavior already existed and no production code changed",
    "coverage-only existing behavior; red-first is N/A because behavior already existed and no production code changed"
  ]) {
    assert.deepEqual(validateTddCloseoutBody(coverageOnlyBodyWith(red)), []);
  }
});

test("points an unjustified coverage-only red cell at the coverage-only form", () => {
  const errors = validateTddCloseoutBody(coverageOnlyBodyWith("coverage-only existing behavior"));

  assert.ok(
    errors.some((error) =>
      error.includes("a coverage-only existing-behavior row must justify the skip")
    )
  );
  assert.ok(errors.every((error) => !error.includes("existing-test contract-change expectation")));
});
