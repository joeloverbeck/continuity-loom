...
const citationSpanPattern = /`[^`\n]*`|"[^"\n]*"|'[^'\n]*'/g;

// Returns the offending placeholder token, or "" when the value is resolved. Backticked
// and quoted spans are citations, not template placeholders, so a real artifact, test, or
// domain token that happens to contain TBD/TODO/pending/unknown or angle brackets stays
// admissible; a bare template placeholder is still rejected.
export const unresolvedEvidenceToken = (value) => {
  if (!value || !value.trim()) return "empty";
  const outsideCitations = value.replace(citationSpanPattern, " ");
  if (/^\s*<[^>]*>\s*$/.test(value.trim())) return value.trim();
  return (
    outsideCitations.match(/\b(?:TODO|TBD|pending|unknown)\b/i)?.[0] ??
    outsideCitations.match(/<[^>]+>/)?.[0] ??
    ""
  );
};
...
// The coverage-only classification needs the label plus a red-first N/A justified by the
// behavior already existing; ordinary connective words between them do not change the
// claim, so they do not change admissibility.
export const hasCoverageOnlyExistingBehaviorEvidence = (value) =>
  /\bcoverage-only existing behavior\b/i.test(value) &&
  /\bred-first\b[^;]{0,24}\bN\/A\b[^;]{0,24}\bbecause\b[^;]{0,24}\bbehavior already existed\b/i.test(value);
...
export const hasConcreteRedEvidence = (value) => {
  ...
  if (hasCoverageOnlyExistingBehaviorEvidence(normalized)) return true;
