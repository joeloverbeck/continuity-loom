...
|---|---|---|---|
| #369 | issue #369 AC1-AC2; sequence: settings load -> author edits a value -> value persisted -> reload still shows the value -> author clears it -> reload shows it empty, that whole order asserted end to end by the four settings tests in packages/web/src/settings.test.ts | settings diff and tests | none |
| #370 | issue #370 AC1; sequence: build the request -> inspect it -> derive the finalized properties -> transport that same object once, order and final object identity asserted by the presenter regression tests in packages/core/src/request.test.ts | request diff and tests | none |
| #371 | issue #371 AC1; sequence: N/A because principles and ADR conformance is a static property of the diff, so no order of events can change whether it holds | docs diff | none |
