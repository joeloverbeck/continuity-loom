...
Do not erase found-and-fixed history by reporting `no findings`. Make `Findings found` equal the ledger row count and reconcile initial/final counts. Accepted residuals require one record each with `Accepted residual:`, `Axis:`, `Source:`, `Rationale:`, `Revisit trigger:`, and the literal `unhandled findings none beyond accepted residuals`.
