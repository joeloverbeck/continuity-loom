...
test("rejects HTML-like angle tokens and documents shared identity safety", () => {
  ...
  const implementationCloseout = readFileSync(
    new URL("../implementation-closeout.md", import.meta.url),
    "utf8"
  );
  const skillContract = `${skill}\n${implementationCloseout}`;
  ...
  assert.match(skillContract, /use validator-safe prose/);
  ...
  for (const contract of [skillContract, implementTemplate]) {
  ...
  for (const contract of [skillContract, fallback]) {
  ...
  assert.match(skillContract, /complete normal review evidence once in the shared evidence core/);
  assert.match(skillContract, /validate that core with the normal review contract/);
  assert.match(skillContract, /must not repeat normal review evidence or claim to revalidate it/);
  assert.match(skillContract, /multi-pass review or more than one TDD review fix/);
  assert.match(skillContract, /--evidence-input <evidence\.json> --immediate-fix --tdd-parent-rollup/);
  assert.match(skillContract, /\.\.\/implement\/references\/closeout-templates\.md/);
  assert.match(skillContract, /Do not hand-copy those derived fields/);
