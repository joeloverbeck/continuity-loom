...
Title the issue `PRD: <name> — <key mechanisms>`, matching prior PRDs. Preserve the same published title punctuation as same-kind exemplars, including the em dash in `PRD: <name> — <key mechanisms>` when that is the house style. When intake found no same-kind exemplar, this exact title form is the bootstrap house style. ASCII-only local editing defaults do not require downgrading tracker issue titles unless the tracker or CLI rejects the character. The issue title is set via `--title`; the body (or `--body-file`) begins at the preamble and must not repeat the title as an H1 heading.
...
Bodies drafted before issue numbers exist reference siblings generically ("the first program PRD"); after creation, resolve those references to concrete numbers via a sequence comment on each issue or a body edit, stating each PRD's position and any label flip condition. If sequence comments are used, post the same concrete sequence comment to every issue in the program and verify exactly one byte-identical copy on each issue with the tested verifier below. A zero or duplicate count fails closeout; do not repost merely because readback failed. If body edits are used instead, verify each edited body contains the concrete sibling numbers before final reporting.

```sh
comments_json="$(gh issue view "$issue_number" --json comments)" || {
  printf 'sequence-comment tracker read failed for #%s\n' "$issue_number" >&2
  exit 1
}
test -n "$comments_json" || {
  printf 'sequence-comment tracker read returned empty output for #%s\n' "$issue_number" >&2
  exit 1
}
printf '%s\n' "$comments_json" \
  | node .claude/skills/to-prd/scripts/verify-sequence-comment.mjs --expected-body "$SEQUENCE_COMMENT"
```
...
When no same-kind exemplar exists, choose a verified existing repository type label by issue kind: `bug` for a confirmed defect, `enhancement` for net-new behavior, and `documentation` for doc-only work. Do not invent a type label. If the appropriate type label is absent, include the exact proposed name, color, description, and creation command in Step 2; without that ratification, stop before creation.

For a missing canonical triage label, use the exact creation metadata in `docs/agents/triage-labels.md` and include the resulting `gh label create` command in Step 2. Do not infer colors or descriptions from another repository, and do not rewrite metadata on an existing label during ordinary PRD publication.
...
- **Ready.** If no provisional, unratified, timed-out, or open-to-veto decisions remain, verify the `ready-for-agent` label exists (create it only when the exact triage-doc metadata and command were ratified at Step 2; a verification earlier in the same session suffices), then apply it. No additional triage round-trip is needed, but any pre-label acceptance checks imposed by the project's issue-tracker doc still apply.
- **Needs triage.** If any provisional, unratified, timed-out, or open-to-veto decision remains, do not apply `ready-for-agent`; apply `needs-triage` instead, verify that label exists under the same creation rule, and record in the final report that the PRD is not AFK-ready until those decisions are ratified.
...
Apply the chosen triage label alongside the non-triage type label confirmed from same-kind exemplar metadata. When no same-kind exemplar exists, use the verified bootstrap type-label mapping above instead.

## Browser-visible guidance checklist gate

In this repo, a PRD changing browser-visible behavior, browser workflow or navigation, prompt preview or inspection, user-initiated external-model invocation, or candidate/accepted-output handling must satisfy `docs/agents/issue-tracker.md`'s browser-visible guidance acceptance checklist before `ready-for-agent` is applied.

Treat this as a checked gate, not a self-assertion: before applying `ready-for-agent`, enumerate the canonical items between the issue-tracker document's checklist markers and confirm each maps to a PRD section that carries it (the provenance preamble, Problem Statement/Solution, User Stories, Implementation Decisions, Testing Decisions, Principles, or Further Notes). The validator reads those items from that authority; do not maintain a second checklist in this skill.
...
Distinguish an item whose condition is outside the PRD's scope from an item that applies but is unhomed. An inapplicable item is `N/A - <specific reason>` and does not force `needs-triage`. Only an applicable item with no concrete home is unmet; if any applicable item is unmet, apply `needs-triage` instead and name it in the final report.
