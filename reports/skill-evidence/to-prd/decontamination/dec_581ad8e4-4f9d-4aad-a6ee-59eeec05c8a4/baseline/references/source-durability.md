...
For a ratified-but-unwritten deliverable path, the validator policy has no passing category: listed in `approvedSources` it fails the Git durability gate, listed in `disallowedSources` its presence fails validation, and unlisted it fails as an unexpected local source. Therefore do not let such a path appear in the body in extractable form at all. Carry the exact name in a durable cited source such as the determination artifact, describe the deliverable in body prose without an extractable path, and mark it pending authoring and publication in Further Notes.
