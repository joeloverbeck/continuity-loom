...
test("closeout guidance documents structured input, safe splits, and private-fixture provenance", () => {
  ...
  assert.match(templates, /## Structured evidence/);
  assert.match(templates, /Use either `auditRows` or `--audit-input`, not both/);
  ...
  assert.match(templates, /The core owns TDD, review, browser, identity, Principles, and preflight evidence/);
  assert.match(implementationEvidence, /Keep private fixture paths private/);
  assert.match(implementationEvidence, /stable logical ID, content hash, and provenance/);

  const documentedEvidence = JSON.parse(
    templates.match(/Minimal shape:\n\n```json\n([\s\S]*?)\n```/)?.[1] ?? ""
  );
  assert.deepEqual(Object.keys(documentedEvidence), [
    "auditRows",
    "tddRows",
    "tddReviewFixes",
    "reviewFindings"
  ]);
  assert.equal(documentedEvidence.auditRows[0].checkId, "AC1");
});
