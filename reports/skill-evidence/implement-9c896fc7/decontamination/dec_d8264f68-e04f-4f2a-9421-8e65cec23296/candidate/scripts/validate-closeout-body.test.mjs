...
test("closeout guidance delegates review shape and keeps the executable normal validator", () => {
  ...
  assert.match(template, /Use the executable helpers as the authority for body syntax/);
  assert.match(template, /Use `--review fallback` only when the canonical `code-review` workflow selected fallback/);
  assert.match(template, /validate-review-normal-body\.mjs/);
  assert.match(template, /--acceptance-manifest \/tmp\/implement-acceptance-manifest\.json/);
  assert.match(gates, /canonical `code-review` evidence/);
  assert.doesNotMatch(template, /Review fallback: <fallback line/);
});

test("implementation guidance preserves the transferable safety contracts", () => {
  ...
  assert.match(skill, /compare current `HEAD`, worktree, governing authority, ledger/);
  assert.match(skill, /do not edit, stage, or revert/);
  assert.match(skill, /Tracked implementation evidence and publishable closeout evidence are separate sinks/);
  assert.match(skill, /review must cover the actual final `HEAD`/i);
  assert.match(skill, /exact-read all final issue states and commit visibility/);
  assert.match(skill, /--emit-preflight`.*--mutation-ready/);
  assert.match(evidenceGuide, /never raw-copy a live store/i);
  assert.match(evidenceGuide, /artifact disposition/);
  assert.match(evidenceGuide, /## Verification ledger/);
  assert.match(evidenceGuide, /\| Exact command \| Observed result\/counts \| Run count \| Represented SHA\/tree \|/);
  assert.match(evidenceGuide, /Publish only final-tree ledger rows/);
  assert.match(evidenceGuide, /Do not delete any artifact still named as current evidence/);
  assert.match(scopeGuide, /capture-github-issues\.mjs <issue\.\.\.> --output \/tmp\/implement-issues\.json/);
  assert.match(scopeGuide, /On resume, compare current `HEAD`, worktree, governing authority/);
  assert.match(template, /build-closeout-body\.mjs/);
  assert.match(template, /--scope issue-set --anchor <issue>/);
  ...
  assert.match(template, /--expected-final-sha "\$\(git rev-parse HEAD\)"/);
  assert.match(template, /--emit-preflight --mutation-ready/);
  assert.match(reviewGuide, /Tracked implementation reports are SHA-independent evidence sources/);
  assert.match(reviewGuide, /Refresh final SHA and actual remote reachability/);
  assert.match(reviewGuide, /machine-derived `Accepted residuals:` summary verbatim/);
  assert.match(template, /verify-github-comment-body\.mjs/);
  assert.match(template, /65,536 UTF-8 bytes/);
  assert.match(template, /unique `\(issue, seam\)` row/);
  assert.match(gates, /--emit-preflight`, and `--mutation-ready` are mandatory/);
  assert.match(gates, /exact UTF-8 body match/);
  assert.match(gates, /Post-comment verification next:/);
  assert.match(gates, /not mutation-ready/);
  assert.match(gates, /actual remote reachability/);
  assert.match(childGuide, /## Oversized evidence/);
  assert.match(childGuide, /shared core/);
  assert.match(childGuide, /split-core workflow in `closeout-templates\.md`/);
  assert.match(childGuide, /Exact-read every affected issue state/);
});
