...
Low-headroom split exception: when the mandatory size plan reports `low-headroom` or `exceeds-limit`, the split-evidence order overrides the parent-first order in steps 1-2 below. Build one full implement closeout body as the shared evidence core and give it a disjoint subset manifest. Build and validate its first-post form with `--split-core-preindex`; its exact index line must say that chunks are not indexed and its `Body file(s) inspected:` value must say that linked chunk bodies do not exist in the pre-index state. Immediately before the first tracker mutation, validate that core with every applicable nested validator and the implement validator's `--emit-preflight --mutation-ready --split-core-preindex` gate, then post and exact-read it. Build each remaining disjoint subset with `--audit-chunk --shared-evidence-core-url <verified-core-comment-url>`; validate the chunk with the implement validator's matching `--closing --audit-chunk` contract and do not repeat TDD, review, browser, identity, or preflight evidence. Post and read back every disjoint evidence chunk before closing any issue, and use `verify-github-comment-body.mjs` against every returned URL. Once every chunk URL is verified, patch the shared core with an exact `Linked acceptance-audit chunks:` HTTPS URL index and the final inspection claim, revalidate it with `--split-core-final` against only its own subset, and exact-read the patched core again. The final-state builder uses repeatable `--linked-audit-chunk-url <verified-https-url>` values. Fixed-template child comments can then cite that one indexed core URL before resuming at step 3. Never put unresolved core/chunk URLs in a posted body or claim that the core validation covered rows held in the chunks.
