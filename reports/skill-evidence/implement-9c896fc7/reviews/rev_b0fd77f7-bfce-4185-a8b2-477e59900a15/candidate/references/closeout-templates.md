...
audit bodies, a tracker-only zero-code-diff close, `/tmp` body files, closeout
preflight scratchpads, or final body-check commands.

## Tracker-Only / Zero-Code-Diff Closeout

Use this ONLY when the closeout represents work that changed **zero repository files** — a
tracker-only PRD ratification, wording publication, or administrative close whose
implementation diff is empty and for which no acceptance-manifest scaffold applies. If **any**
repository file changed, including a docs-only or one-line code change, this recipe does not
apply: use the normal per-issue or parent-rollup path with real verification, review, and
evidence. This recipe never authorizes skipping real evidence for a change that touched code,
docs, fixtures, or any tracked file.

The closing validator still requires the full closeout shape, so a zero-diff close pays a
git-only verification ledger against the current HEAD plus each code-implementation-shaped
field set to its exact validator-accepted N/A disposition. That scattered N/A shape is the
whole cost this recipe removes: copy the body below, substitute every bracketed value (current
HEAD SHA, issue number, exact acceptance wording, a real tracker comment reference), inspect
it, then run the single validator command. It clears the mutation-ready gate in one round with
no field-by-field discovery.

```markdown
Tracker-only closeout for #<issue>

Final SHA: <current HEAD sha>
Verification:

| Exact command | Observed result/counts | Run count | Represented SHA/tree |
|---|---|---|---|
| `git rev-parse HEAD` | passed; printed <current HEAD sha> | 1 | `<current HEAD sha>` |
| `git status --short` | passed; clean working tree, no code diff | 1 | `<current HEAD sha>` |
...
|---|---|---|---|
| #<issue> | AC1 - <exact acceptance wording> | atoms: <ratified or published atom>; proof surfaces: <real tracker comment URL or #issue reference>; sequence: N/A because criterion is not sequence-sensitive | satisfied |

Closeout preflight:
- Audit sink: this closeout comment on issue #<issue>
...
Closeout gate passed: audit sink this closeout comment on issue #<issue>; review evidence Review: code-review N/A tracker-only empty diff; TDD evidence N/A; final SHA <current HEAD sha>; Principles/ADR conformance N/A; Local-only SHA sentence N/A; child states verified N/A; browser evidence N/A.
...
Substitute every bracketed value, then validate the inspected body with one command using the
current HEAD as both the represented and expected SHA, immediately before the first tracker
mutation:

```bash
node .claude/skills/implement/scripts/validate-closeout-body.mjs "$body" --closing --expected-final-sha "$(git rev-parse HEAD)" --emit-preflight --mutation-ready
```

Copy the emitted `Closeout preflight:` block, `Closeout gate passed:` line, mutation-ready
confirmation, and `Accepted residuals:` summary verbatim into the conversation before the
mutation. If an in-scope issue has a `## Principles` section, add `--principles` and replace
the `Principles/ADR conformance:` N/A form with the real conformance sentence. If the
represented HEAD is not remote-reachable, add `--local-only` and replace the `Local-only SHA:`
lines with the full copy-ready `Local-only SHA:` sentence. Never use this recipe to shorten a
close that changed any tracked file.
