...
Parent PRD coverage: ${scopeMode === "issue-set"
    ? "N/A because this is a sibling issue set with no parent PRD."
    : scopeMode === "standalone"
      ? "N/A because this is one standalone issue with no parent PRD."
      : "<parent row present, exact audit rows cited, or N/A>"}
...
const resolveScope = ({ scopeMode = "parent", parentIssue, anchorIssue }) => {
  if (!new Set(["parent", "standalone", "issue-set"]).has(scopeMode)) {
    throw new Error("scope mode must be parent, standalone, or issue-set");
  }
  if (scopeMode === "parent") {
    if (!Number.isInteger(parentIssue) || parentIssue <= 0) {
      throw new Error("parent issue must be a positive integer");
    }
    if (anchorIssue !== undefined) throw new Error("anchor issue applies only to standalone or issue-set scope");
  ...
  if (parentIssue !== undefined) throw new Error("use either parent scope or anchored scope, not both");
  return { scopeMode, subjectIssue: anchorIssue };
};

const scopeHeading = ({ scopeMode, subjectIssue }) => {
  if (scopeMode === "issue-set") return `sibling issue set anchored at #${subjectIssue}`;
  if (scopeMode === "standalone") return `standalone issue #${subjectIssue}`;
  return `#${subjectIssue}`;
};

const validateScopeManifest = (manifest, scope) => {
  if (scope.scopeMode !== "standalone") return;
  if (manifest.issues.length !== 1 || manifest.issues[0].number !== scope.subjectIssue) {
    throw new Error(
      `standalone scope requires exactly one manifest issue matching anchor #${scope.subjectIssue}`
    );
  }
};
...
const renderCloseoutBodyScaffold = (manifest, options) => {
  ...
  validateScopeManifest(manifest, scope);
  ...
  if (scope.scopeMode !== "parent" && fixedChildMode !== "none") {
    throw new Error("only parent scope can use fixed-child modes");
  }
...
Post-closeout issue-set verification: <exact CLOSED/open states by issue number>`
    : scope.scopeMode === "standalone"
      ? `Standalone issue state snapshot before closeout: <exact OPEN state for #${scope.subjectIssue}>

Post-closeout standalone verification: <exact CLOSED/open state for #${scope.subjectIssue}>`
...
Post-child closure verification before parent closeout: <exact issue states or pending follow-up>`;
  ...
  const fixedChildInspection = scope.scopeMode === "issue-set"
    ...
    : scope.scopeMode === "standalone"
      ? "N/A because standalone issues do not use fixed-child closeout"
...
if (isCli) {
  ...
  try {
    ...
    } else if (scopeMode === "issue-set" || scopeMode === "standalone") {
      if (!anchorText || anchorText.startsWith("--")) throw new Error(`--scope ${scopeMode} requires --anchor`);
      if (parentText !== undefined) throw new Error("use either --parent or --scope with --anchor, not both");
    } else {
      throw new Error("--scope must be standalone or issue-set when supplied");
    }
    ...
    const { body, sizePlan } = buildCloseoutBodyPlan(manifest, {
      ...
      anchorIssue: scopeMode === "parent" ? undefined : Number(anchorText),
