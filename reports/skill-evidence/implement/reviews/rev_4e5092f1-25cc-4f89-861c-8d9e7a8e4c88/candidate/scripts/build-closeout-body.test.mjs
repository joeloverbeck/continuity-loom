...
test("closeout scaffold CLI supports one truthful standalone issue and rejects mismatched scope", () => {
  const directory = mkdtempSync(join(tmpdir(), "implement-standalone-scaffold-test-"));
  const standaloneManifest = selectAcceptanceManifest(manifest, ["368"]);
  const manifestPath = join(directory, "manifest.json");
  const outputPath = join(directory, "closeout.md");
  writeFileSync(manifestPath, JSON.stringify(standaloneManifest));
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.match(body, /^Implementation closeout for standalone issue #368$/m);
  assert.match(body, /^Parent PRD coverage: N\/A because this is one standalone issue with no parent PRD\.$/m);
  assert.match(body, /^- Parent rollup URL: N\/A because this is one standalone issue with no parent PRD$/m);
  assert.match(body, /^- Fixed child inline close comment: N\/A because standalone issues do not use fixed-child closeout$/m);
  assert.doesNotMatch(body, /sibling issue set/);
  ...
  assert.match(mismatch.stderr, /exactly one manifest issue matching anchor #901/);
  rmSync(directory, { recursive: true, force: true });
});
