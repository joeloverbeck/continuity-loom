...
## Closeout Scope and First-Pass Field Contracts

Classify the tracker relationship before invoking the scaffold builder:

- One issue with no parent or in-scope siblings: use
  `--scope standalone --anchor <issue>`. The supplied manifest must contain
  exactly that one issue. Never use parent or sibling wording for this case.
- Two or more in-scope siblings with no parent PRD: use
  `--scope issue-set --anchor <audit-anchor>`.
- A real parent/child family: use `--parent <parent>` and the applicable
  fixed-child state.

Fill validator-sensitive fields from their literal contracts before the first
validator run. Preserve the concrete facts; the tokens below are structural,
not permission to invent evidence:

- **Backend process currentness** must contain the literal tokens `server
  command`, `watch/reload mode`, `process/port ownership`, `restart proof` (or
  `reload proof`), and an `expected ... API ... probe`.
- **Acceptance atom map** must literally start the claim with `all rows` and
  name both `atoms` and `proof surfaces`.
- **Pre-red evidence reference** needs a stable durable sink, an anchor word
  (`section`, `heading`, `line`, or `row`), and chronology stating that the
  record precedes the first red command. A published body must not expose a
  local staging path.

These are first-draft composition contracts. Do not wait for a validator
failure before applying them.
...
## Standalone-Issue Closeout

For exactly one issue with no parent PRD and no in-scope sibling, use the same
manifest-backed audit and scaffold workflow without inventing a relationship:

```bash
node .claude/skills/implement/scripts/build-closeout-body.mjs /tmp/worldloom-acceptance-manifest.json --audit-input /tmp/worldloom-acceptance-audit.md --output /tmp/worldloom-closeout-901.md --scope standalone --anchor 901 --review normal --size-plan --require-headroom
```

The builder rejects a standalone anchor unless the manifest contains exactly
that issue. Parent/child and sibling fields render as explicit N/A values.
...
Validator-passing field examples: several remaining closeout fields are enforced by exact-token regexes in `.claude/skills/tdd/scripts/tdd-evidence-contract.mjs` and `.claude/skills/code-review/scripts/review-evidence-contract.mjs`, and the natural-language placeholders in the template do not map one-to-one to the required tokens. Apply the first-pass contracts at the start of this reference before validation. For other rejected fields, match one of these copy-paste passing forms (substitute the bracketed specifics), then re-run the validator:
