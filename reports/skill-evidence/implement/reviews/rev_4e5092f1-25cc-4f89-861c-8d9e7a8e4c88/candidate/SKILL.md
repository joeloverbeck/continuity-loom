...
When review produced findings or more than one TDD review fix, create or refresh the structured evidence JSON and build the final closeout scaffold with `--evidence-input <evidence.json> --immediate-fix`. When that evidence includes TDD rows or review fixes, also use `--immediate-fix --tdd-parent-rollup`. Do not hand-copy the derived compact TDD rows, RF rows, review-finding ledger, finding counts, severities, issue/seam accounting, or repeated summaries; let the builder derive them from the single structured input described in [references/closeout-templates.md](references/closeout-templates.md).
...
Classify the closeout scope before invoking the scaffold builder: one issue
with no parent or siblings uses `--scope standalone --anchor <issue>`; 2+
siblings with no parent use `--scope issue-set --anchor <audit-anchor>`; a real
parent/child family uses `--parent <parent>`. Never route a standalone issue
through parent or sibling wording. Use the first-pass field contracts at the
start of [references/closeout-templates.md](references/closeout-templates.md)
while filling the scaffold, before the first validator run.
