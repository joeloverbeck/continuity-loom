...
At handoff, copy the dispatch authorities into `Handoff Standards source inventory:` and `Handoff Spec source inventory:` without adding, dropping, or summarizing entries. Re-read both pre-dispatch fields and reconcile the two sets before reporting zero findings or returning to `implement`.
...
- **Pre-dispatch Spec source inventory**: `<issue #N | concrete spec path | no spec available / N/A because local fallback was used>`
- **Handoff Standards source inventory**: `<the exact same entry set as Pre-dispatch Standards source inventory / N/A because local fallback was used>`
- **Handoff Spec source inventory**: `<the exact same entry set as Pre-dispatch Spec source inventory / N/A because local fallback was used>`
