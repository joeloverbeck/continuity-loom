...
2. **Read alignment documents.**
   - Read `docs/FOUNDATIONS.md` — skip only if it's already in this session's context, and when skipping, name the load mechanism explicitly (e.g., "already in context via direct Read at message N"). It is the constitution: §1–§28 principles plus the §29 alignment / hard-fail checklist (§29.1–§29.8).
