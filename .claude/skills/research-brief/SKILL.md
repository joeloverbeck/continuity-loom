...
Before presenting, confirm both audit-trail announcements were emitted earlier this run: the Step 1 `Classification:` line and the Step 3 online-research run-or-skip one-liner. If either was missed, emit it now. Also confirm every path in the read-in-full list resolves at the fetch-baseline commit before you present it (run the §2-completeness check from Step 6), and that every `§N`/named section anchor you intend to cite *into* a read-list doc resolves at that commit (grep the heading — a path can resolve while the cited section does not): drop or correct any path or anchor that doesn't — predecessor reports often cross-reference files since deleted — so the list you present is the list Session 2 can actually fetch.
