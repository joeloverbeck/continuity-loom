...
- **Completion-bookkeeping deliverable** — a deliverable that performs end-of-phase bookkeeping rather than producing product code: flipping a status line in a sequencing doc (e.g. `docs/requirements-version-1/IMPLEMENTATION-ORDER.md`), adding a "Phase-N implementation note" to governing docs, and archiving the spec (`docs/archival-workflow.md` → `archive/specs/`). Route it into the trailing capstone / cross-cutting-docs ticket rather than producing its own ticket; its acceptance is grep-proofs on the status/note lines plus `test -f` / `test ! -f` on the archival boundary. Recurs in Continuity Loom as the spec's "governing-doc updates on completion" deliverable.
