...
- **Skill dry-run variant** — when a §Verification bullet requires invoking an LLM-driven skill (not runnable from test code), structure the ticket as a hybrid: a *manual dry-run runbook* (in the What to Change section) the implementer follows by hand — fixture copy strategy, skill invocations against the copy, expected post-skill state with verification commands — plus an automated-test subsection for any test-runnable portion (grep negative tests, schema assertions). The acceptance criteria distinguish CI-runnable assertions from implementer-checklist dry-runs.
