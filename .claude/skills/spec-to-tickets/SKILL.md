...
**Non-numbered deliverables**: many specs lack a numbered `§Deliverables` section. Anchor the deliverable enumeration in this precedence order: (1) a numbered `§Deliverables`; (2) absent that, a **per-deliverable** `§Implementation sequencing guidance` / `§Sequencing` ordered list when present — its steps are already work-ordered units and usually map close to one-per-ticket; (3) else the items of a `§Scope` / `§In scope` list, or each distinct named implementation section. Whichever enumeration you anchor on, **cross-check its coverage** against the spec's `§Acceptance` / `§Coverage` / `§Verification` (or equivalent) so no testable rule is left unmapped to a ticket, and reconcile divergent counts (one spec may carry, for the same work, a 4-surface `§Scope`, a 9-step `§Implementation sequencing guidance`, and 13 acceptance criteria) rather than treating any single list as exhaustive. **Per-layer/phase sequencing is the exception, not item (2)'s default case**: when the sequencing list is organized by *layer* (e.g. schema → migration → validation → compiler → UI → docs) cutting across several deliverables rather than by deliverable, do NOT map one ticket per phase — a per-phase ticket leaves a broken intermediate (a strict-schema phase tightened before its migration phase lands; a type member removed in one package while consumers in others remain). Anchor on the deliverable axis (item (1) or (3)) and treat the phase plan as *ordering input* only (see `references/decomposition-patterns.md` §Schema-field-removal vertical for the co-landing reason).
...
**Provisional-table bundling**: when Step-2 Issues each carry a recommended disposition, you MAY present them together with a *provisional* Step-4 table explicitly conditioned on those recommendations, so a single approval satisfies both the per-Issue disposition (HARD-GATE (b)) and the table (HARD-GATE (c)) — provided each Issue's disposition is still stated explicitly and you re-affirm the table if the user's pick diverges from a recommendation. This saves a round-trip without weakening the gate.

## Step 5: Batched Ticket Writes

**Post-approval refinement (mechanical only)**: while composing, you MAY apply a *mechanical tightening* to the approved Step-4 table without re-approval — specifically, removing a `Deps` entry that composition shows is unnecessary, relocating a sub-feature between already-approved sibling tickets of the same deliverable, or adding a `(modify)` of an **already-existing** file that an already-approved deliverable's stated behavior requires (e.g. a navigation deep-link target the deliverable's described behavior implies but the Step-4 table did not enumerate) — provided it adds no ticket, creates no new file, crosses no deliverable boundary, and does not change the ticket count. Disclose every such tightening in the Step 6 summary (original table entry → applied change). Anything beyond that — adding/removing a ticket, changing the count, moving work across deliverables, or introducing a `(new)` file / new deliverable — must round-trip to the user for re-approval per HARD-GATE clause (c). (Touching an additional pre-existing file to fulfil approved behavior is mechanical; creating a new file is not.) A **seam-confirmation read** (see the carve-out below) may itself surface such a mechanical expansion — a pre-existing file the already-approved behavior requires, e.g. a version-bump's hardcoded-assertion sites discovered by a ripple grep: apply it here and disclose it at Step 6. This is a mechanical Files-to-Touch addition, not a table re-plan, so it needs no re-approval — distinct from the carve-out's statement that seam-confirmation does not *re-plan* the table. **Mid-batch re-approval**: if a discovery while composing (e.g. a sibling spec turns out to own a planned deliverable) changes the ticket count or scope, stop the write cadence, re-present the adjusted Step-4 table for re-approval (HARD-GATE clause (c)), then resume from the next unwritten ticket — already-written tickets unaffected by the change need not be rewritten.
