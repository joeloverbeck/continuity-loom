...
- Immediately before the first `gh issue comment`, `gh issue close`, `glab issue close`, or equivalent tracker mutation, run the applicable closing validator with `--emit-preflight --mutation-ready`, then produce its exact `Closeout preflight:` block and exact `Closeout gate passed: audit sink ...` line in the conversation or durable audit sink. A plain successful `--closing` validation says `not mutation-ready` and does not authorize tracker mutation. This is required even when the detailed audit is in a parent rollup body.
...
| Any implement closeout body | `node .claude/skills/implement/scripts/validate-closeout-body.mjs "$body"` | Add `--closing --expected-final-sha "$(git rev-parse HEAD)"` for bodies used to close issues, `--principles` when any in-scope issue or parent PRD has a `## Principles` section, `--local-only` when the final SHA is not remote-reachable, `--review-fallback` when local code-review fallback was used, and `--acceptance-manifest <path>` for issue-family closeout generated from saved exact issue JSON. On the last successful validation immediately before the first tracker mutation, also add `--emit-preflight --mutation-ready` and copy its stdout verbatim into the conversation. Without those flags, successful closing output is explicitly not mutation-ready. |
| Shared evidence core, first-post pre-index state | `node .claude/skills/implement/scripts/validate-closeout-body.mjs "$body"` | Add `--split-core-preindex --acceptance-manifest <core-subset-manifest>` to the applicable closing flags. The body must contain the exact truthful first-post `Linked acceptance-audit chunks:` line and `Body file(s) inspected: shared core body inspected; linked audit chunk bodies do not exist in the pre-index state`. This mode may own `--emit-preflight --mutation-ready` immediately before posting the core. |
| Shared evidence core, final indexed state | `node .claude/skills/implement/scripts/validate-closeout-body.mjs "$body"` | After every chunk URL and stored body are verified, patch the core to a `Linked acceptance-audit chunks:` list with one or more unique concrete HTTPS URLs, change `Body file(s) inspected:` to `shared core body and every linked audit chunk body inspected after URL capture`, and add `--split-core-final --acceptance-manifest <core-subset-manifest>` to the applicable closing flags. Exact-read the patched core again. Do not reuse `--split-core-preindex` after the index exists. |
| Linked acceptance-audit chunk | `node .claude/skills/implement/scripts/validate-closeout-body.mjs "$chunk"` | Add `--closing --audit-chunk --shared-evidence-core-url <verified-https-comment-url> --expected-final-sha "$(git rev-parse HEAD)" --acceptance-manifest <subset-manifest>`. The chunk owns only its exact disjoint acceptance rows; the exact-read shared core owns verification, TDD, review, browser, evidence-identity, Principles/ADR, and mutation-preflight evidence. Do not add `--emit-preflight` or `--mutation-ready` to a linked chunk; the shared core must have passed that gate immediately before the first mutation. |
...
node .claude/skills/implement/scripts/validate-closeout-body.mjs "$body" --closing --expected-final-sha "$(git rev-parse HEAD)" --emit-preflight --mutation-ready --principles --local-only --fixed-child-pending --review-fallback --acceptance-manifest /tmp/worldloom-acceptance-manifest.json
```
...
- Before first tracker mutation hard stop: run the applicable implement closing validator with `--expected-final-sha "$(git rev-parse HEAD)" --emit-preflight --mutation-ready`, then copy its stdout into the conversation or durable audit sink before the first `gh issue comment`, `gh issue close`, `glab issue close`, or equivalent command. The emitted output must contain `Mutation-ready closeout validation passed:` plus all three exact headings/lines below; do not paraphrase or hand-transcribe them. A successful closing validation without this explicit pair is diagnostic only and prints `not mutation-ready`.
  - `Closeout preflight:`
...
Drop only flags whose conditions do not apply; `--expected-final-sha`, `--emit-preflight`, and `--mutation-ready` are required on this last run. If the parent rollup URL has already been captured and the body contains the exact final child inline close comment with that real URL, use `--fixed-child` instead of `--fixed-child-pending`. Copy the validator's emitted `Closeout preflight:` block, `Closeout gate passed:` line, `Post-comment verification next:` reminder, mutation-ready confirmation, and generated accepted-residual summary verbatim into the conversation or durable audit sink before mutation.
...
- If review has accepted residuals, run the implement validator with `--emit-final-summary` (already implied by `--mutation-ready`) and use its generated `Accepted residuals:` line in the final response. Preserve its count and affected axes, cite its durable residual record, and retain `unhandled findings none beyond accepted residuals`; never collapse that outcome to `no findings` or `no remaining findings`.
