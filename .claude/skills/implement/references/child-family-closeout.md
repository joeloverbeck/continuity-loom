...
Low-headroom split exception: when the mandatory size plan reports `low-headroom` or `exceeds-limit`, the split-evidence order overrides the parent-first order in steps 1-2 below. Build one full implement closeout body as the shared evidence core and give it a disjoint subset manifest. Immediately before the first tracker mutation, validate that core with every applicable nested validator and the implement validator's `--emit-preflight --mutation-ready` gate, then post and exact-read it. Build each remaining disjoint subset with `--audit-chunk --shared-evidence-core-url <verified-core-comment-url>`; validate the chunk with the implement validator's matching `--closing --audit-chunk` contract and do not repeat TDD, review, browser, identity, or preflight evidence. Post and read back every disjoint evidence chunk before closing any issue, and use `verify-github-comment-body.mjs` against every returned URL. Once every chunk URL is verified, patch the shared core with an exact `Linked acceptance-audit chunks:` URL index, revalidate it against only its own subset, and exact-read the patched core again. Fixed-template child comments can then cite that one indexed core URL before resuming at step 3. Never put pending core/chunk URLs in a posted body or claim that the core validation covered rows held in the chunks.
