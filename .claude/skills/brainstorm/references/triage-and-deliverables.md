...
- **System spec** — deliverable is the spec in `specs/`; the spec IS the design, so skip the design doc. Default sections when no drafting-rules file exists: Problem Statement, Approach, FOUNDATIONS Alignment, Verification, Out of Scope (add Deliverables / Risks & Open Questions as the subject matter warrants); add a top-of-file note that the default structure was used. The Problem Statement should capture the motivation, evidence, and key "considered X, chose Y because Z" decisions from the brainstorm. **Spec-ID assignment**: scan `specs/` for the highest existing `SPEC-NNN` and claim the next integer (first spec is `SPEC-001`). The Step 6 menu is mandatory (post-deliverable phase): offer reassess / decompose into tickets / implement / done.
