...
- **Implementation tickets** — deliverable is the ticket file(s) in `tickets/`; the tickets ARE the design. If a ticket template exists, follow it; otherwise use a minimal Title / Context / Acceptance Criteria / Verification shape and note the template's absence. **Namespace**: tickets use per-initiative prefixes (e.g. `COMPILER-NNN`, `VALIDATE-NNN`), not one global sequence. Scan `tickets/` **and `archive/tickets/`** for existing prefixes and reuse an established one; when reusing a prefix, take the highest existing number under it across both locations — the `tickets/` → `archive/tickets/` workflow routinely empties `tickets/`, so a `tickets/`-only scan can re-claim `-001` and collide with archived history (mirroring the System-spec ID rule above). For a fresh namespace, derive a short uppercase initialism from the deliverable's primary subject and start at `<PREFIX>-001`; still glance at `archive/tickets/` to avoid a confusing collision with an archived prefix. Cite the namespace choice in the deliverable lead so the user can redirect. Each `file:line` citation written into a ticket's Assumption-Reassessment block must be operator-verified by Read/grep, not relayed from a sub-agent report whose line ranges may be off — a downstream implementer trusts it (mirroring the System-spec premise-verification rule above). Ticket *creation* presents the Step 6 menu; ticket *update in place* is inline-completed — skip the menu and summarize the delta.
