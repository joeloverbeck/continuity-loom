...
- When the candidate concerns module depth, seams, interface shape, or architecture vocabulary, load the repo's relevant design-vocabulary or architecture skill as an authority.
- When the invocation names a supporting skill or wrapper, load it during intake so its judgment shapes the branches and its result has a recap home.
