...
**Inline user hint (optional)**: text accompanying the path — a parenthetical, post-dash note, or follow-on message (e.g. `specs/SPEC-004-validator.md (Note: I'm worried some validators are too brittle)`) — is an audit-lens constraint. It shapes severity assignment at Step 5 and may reframe Questions at Step 6; it is NOT a second path argument and does NOT override FOUNDATIONS alignment or approved recommendations. When a hint materially shaped a finding's classification, cite it in the Step 6 presentation. A hint that would force a FOUNDATIONS hard-fail is flagged as a CRITICAL Issue, not applied. A hint that instead names a document or path is a **reference pointer**, not a severity lens: promote the named doc into the Step 1–2 mandatory reads and treat it as a Step 3 cross-validation authority. Two flavors route to different substeps: (a) a *naming-authority* pointer ("rely on / per / see X", e.g. the canonical source for deliverable names) feeds §3.7 Bidirectional symbol-name consistency; (b) an *overlooked-impact-surface* pointer ("consider the impact in X", "changes will be needed in X and related code") feeds §3.6 Downstream Consumers plus the doc-sync completeness check — the named doc is a surface the spec's blast radius reaches but may have omitted, not a naming authority.
...
**Per-deliverable already-landed**: when one deliverable shipped while others remain pending, the spec stays (a)/(b)/(c) — reframe just that deliverable as historical, cite the delivering commit/spec — or the landing location (`<path:line>`) when the landed content is directly observable in the tree — and route residual sub-tasks to a deferred note. Do not flip the whole spec to (d).
...
**Correction specs**: when a spec prescribes target behavior for an *existing* implementation (e.g. a regression-plan "Phase N correction" that rewrites validators or rules already in the tree), classify **(b)** and add a baseline-vs-delta check — grep each prescribed symbol/string/code, and for every one that already matches the target end-state, reframe it as "already present at `<path:line>`; remaining work is the delta." This stops a correction spec that reads greenfield from being decomposed into rebuild tickets. See `references/codebase-validation.md` §3.6.
...
- **Source documents** — when the spec cites an external source (a report under `reports/`, a brainstorm output under `docs/plans/`, a research report) in its Problem Statement / Motivating Evidence / Approach, extract the path AND enumerate its actionable claims at Step 2 itself (read the source; for oversized docs use targeted greps with permissive anchoring). Tag each claim's adjudication status (accept / reject / defer / unadjudicated) by scanning the spec's Approach / Deliverables / Out of Scope. This feeds Step 3.10. A source doc that carries quantitative measurements rather than actionable claims (a baseline or example artifact) has nothing to adjudicate this way — spot-verify its metrics per §3.0 instead. A cited **external web URL** (academic paper, provider docs) is *not* a local source document: note it, and adjudicate only when a normative claim the spec rests on hinges on it — then verify that claim via WebFetch; otherwise record it as `external web citation, not a local source doc` and do not treat it as a §3.10 source.
...
**Reference-count checkpoint**: before Step 3, emit a one-line note with the exact reference count and the tracking decision — e.g. `Reference count: 12 — mental tracking sufficient` or `Reference count: 23 — TaskCreate recommended`. Use an exact integer (not `~20` or `20+`); the >15 threshold-decision must be reproducible. **Exception — estimate acceptable**: for doc/amendment specs whose references are doc-section anchors rather than discrete code symbols, *or* for code specs where discrete symbols mix with closed-set enums such that a precise tally is arbitrary (apply the closed-set-counts-as-1 rule first), an estimate plus the tracking decision is acceptable — the reproducibility requirement then binds the >15 *decision*, not the precise count (e.g. `Reference count: ~60 doc-section anchors — clustered, mental tracking sufficient`, or `Reference count: ~32 (two focus-tag closed sets counted as 2) — clustered, mental tracking sufficient`). For specs with >15 references spanning unrelated areas, consider `TaskCreate` per-reference tracking; for tightly-clustered sets, mental tracking is acceptable. A fixed closed set checked by one presence grep counts as 1 reference, not N. In a single-pass non-plan run, this note and the source-document engagement note below may appear in the Step 6 report header instead of inline before Step 3, provided the tracking decision was genuinely made before validation began.

**Source-document engagement checkpoint** (when source documents are cited): emit a one-line note naming each source and its per-document adjudication counts — `Source-document engagement: <doc>: N claims enumerated, M adjudicated (accept / reject / defer), (N-M) unadjudicated flagged as findings.` When no source document is cited, emit `Source-document engagement: N/A — no external source cited`. When a cited source document cannot be resolved or read, emit `Source-document engagement: <doc> — CITED BUT MISSING` and raise a finding (see §3.10). When a cited source doc's content is quantitative measurements rather than actionable claims (a baseline or example artifact — e.g. a compiled-prompt example cited for its size/percentage metrics), adjudicate it via §3.0 baseline spot-verification and record `Source-document engagement: <doc>: N measurements spot-verified (M corroborated / K stale)` instead of the accept/reject/defer counts (see §3.10 **Measurement-baseline source docs**).
