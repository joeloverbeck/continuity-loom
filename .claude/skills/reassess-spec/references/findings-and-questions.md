...
- **Issues**: factually wrong, stale, violates FOUNDATIONS / trips a §29 hard-fail, or proposes redundant deliverables when existing infrastructure suffices. Blocks ticket decomposition.
...
For each finding, record: what the spec says (or omits); what the codebase actually has (with file paths and line references); the recommended change.
...
**Pre-finalization site-counting**: for an Issue or Improvement that corrects a stale claim OR renames/eliminates a token (op-name, type-name, command-name, mechanism token) restated across multiple spec sections, grep the spec for ALL restatements of the claim/token BEFORE finalizing the finding's recommendation. List each site in the recommendation (e.g. `apply at §Approach §B + §Verification step 3 + §Risks R-2`). Use a bare-token grep with `-i` (not the specific phrasing the finding expects to remove) to catch syntax variants. Audit-time enumeration is cheaper than the late-discovery recovery that the Step 7 pre-edit grep and Step 8 post-apply grep otherwise have to absorb.

## Step 6: Present Findings

**Redesign-count checkpoint** (before drafting): count deliverables whose approach was materially changed — eliminated, replaced with a different mechanism, or restructured beyond a refinement. Express as `N / total`, where `total` is the **pre-reassessment** deliverable count (dropped deliverables stay in the denominator; added ones increase it; use the same denominator through every emission). Emit the ratio as a one-line note in the `### Classification` block regardless of whether the flag fires. If `N/total > 50%`, the Substantial Redesign Flag section MUST appear immediately above Questions; otherwise omit it.

Material changes: elimination of a deliverable; replacement of its mechanism; restructuring that changes which modules/files it touches; changing the read/write direction of data flow; introducing a new authoritative state where the original was a derived view (or vice versa). NOT material: field renames, type-shape adjustments that preserve the deliverable's role, signature-preserving reorderings, prose rewording. A test/verification deliverable that re-anchors *only because* its drivers were redesigned counts once with its driver, not separately. For a multi-item inventory (a validator table, an enumerated rule set), count each materially-changed sub-item toward the numerator but the container as one deliverable in the denominator.
...
### §29 Hard-Fail Check
[Only if a §29 group is engaged. One line per engaged group:]
- §29.N <group>: pass | N/A | **flag** (reason)
...
- **Option fidelity**: each option that names a type/field/function must cite its current definition (grepped at presentation time), not a summary. The user's approval binds to the option label, so an imprecise label produces an ambiguously approved fix.
