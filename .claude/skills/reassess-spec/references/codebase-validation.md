...
- **Recurring-term/count sweep** — when an amendment changes a term or count that recurs in the target doc (e.g. "five surfaces" → "six," a renamed section, a renumbered list), grep the *whole* target doc for the old term/count and verify the amendment plan covers **every** occurrence, not just the headline edit. An uncovered occurrence is an Issue: HIGH when it leaves a hard-fail item, alignment-checklist bullet, or normative statement contradicting the new doctrine (an orphaned bullet); MEDIUM for a stale cross-reference in descriptive prose. This is the actionable form of the constitutional-amendment carve-out's "every restated rule moves together" (SKILL.md Guardrails; `foundations-alignment.md` §4.4).
...
**Doc-sync vs. registered authorities**: when a spec touches a doc-governed domain (it updates, or should update, authority docs), list the registered authorities for that domain from `docs/ACTIVE-DOCS.md` and verify the spec's doc-sync / authority-update plan covers each affected one. A registered authority the spec's changes affect but its doc-sync list omits is a HIGH coverage gap — especially one with no conformance test, which would then drift silently. (`docs/ACTIVE-DOCS.md` is the project's authority map per the root `CLAUDE.md`; the skill otherwise consults it only in the §Retroactive archiving branch.)
...
**Reversal of a completed predecessor**: when the spec deliberately overturns a *completed* predecessor ticket/spec's settled decision (e.g. it reverses a shipped ticket's chosen mechanism — short citation keys replacing a predecessor's full-label keys), verify (1) the predecessor's original problem stays solved under the new mechanism — grep the predecessor for the goal it secured and confirm the new design preserves it, not merely that it differs; (2) the reversal is cited explicitly with its decision provenance (which prior decision, what authorizes the overturn — a user sign-off, a sibling spec); (3) any artifact the predecessor created that the reversal strands (a now-orphaned field, doc line, golden assertion, or test) is addressed or noted out of scope. A reversal that silently drops the predecessor's goal is a HIGH Issue (regression risk); a reversal lacking explicit provenance is a MEDIUM Improvement. This is distinct from a **Correction spec** (SKILL.md Pre-Process / §3.6 baseline-vs-delta), which fixes an implementation that *drifted* from intent rather than overturning the intent itself.
...
**Measurement-baseline source docs**: when a cited source doc's content is quantitative measurements (file sizes, token/percentage counts, an example artifact's metrics) rather than actionable accept/reject/defer claims, adjudicate it through §3.0 **Embedded quantitative baseline claims**, not the enumerate-and-adjudicate model above: spot-verify each measurement with `wc` / `grep -c` / `find` and record corroborated-vs-stale, not accept/reject/defer. The Step 2 engagement note then uses the measurement form (`N measurements spot-verified (M corroborated / K stale)`). A doc that mixes measurements with actionable claims gets both treatments.
...
**Uncited originating source discovery**: before applying the skip below, `ls reports/ docs/plans/` for a filename sharing the spec's distinctive slug token (e.g. `SPEC-023-author-private-story-notes` → `archive/reports/author-private-story-notes-research-brief.md`). A research brief or brainstorm output that obviously birthed the spec but goes uncited is a LOW provenance Improvement — recommend the spec cite it for decomposition traceability. This is discovery only, not adjudication: the doc is uncited, so there are no claims to enumerate and the accept/reject/defer model does not run against it.

**Skip** when: (c) refactor classification, or no external source document **and no content-supplying predecessor spec** is cited (self-originating specs are scoped by 3.0–3.9 alone — though the uncited-source discovery sweep above still applies before skipping).
