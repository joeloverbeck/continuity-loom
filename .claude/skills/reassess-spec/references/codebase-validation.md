...
**Uncited originating source discovery**: before applying the skip below, `ls reports/ docs/plans/` for a filename sharing the spec's distinctive slug token (e.g. `SPEC-023-author-private-story-notes` → `reports/author-private-story-notes-research-brief.md`). A research brief or brainstorm output that obviously birthed the spec but goes uncited is a LOW provenance Improvement — recommend the spec cite it for decomposition traceability. This is discovery only, not adjudication: the doc is uncited, so there are no claims to enumerate and the accept/reject/defer model does not run against it.
