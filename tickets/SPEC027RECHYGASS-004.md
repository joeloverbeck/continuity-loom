...
**Deps**: SPEC027RECHYGASS-002, SPEC027RECHYGASS-003
...
3. **Cross-artifact boundary under audit.** Consumes `StoryRecordHygieneSnapshot`/`HygieneRecord` from SPEC027RECHYGASS-002; renders the contract authored in SPEC027RECHYGASS-003 (same revision, §8). `docs/compiler-contract.md` (replace-targets verified at `:5` version pin, `:15` determinism sentence, `:21` `## 2.`) and `docs/story-record-schema.md` (insert §9.3 after `### 9.2 EMOTION` `:950`, before `## 10.` `:991`) co-land here per §8.
