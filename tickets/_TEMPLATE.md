...
**Engine Changes**: <None|Yes — list affected or newly-introduced skills, docs, schemas, or code surfaces>
...
5. <If this ticket touches skill HARD-GATE semantics, fail-closed validation, or deterministic-compilation surfaces: name the exact enforcement surface and confirm the change does not weaken the secret firewall (§15) or break deterministic compilation (§8)>
6. <If this ticket extends an existing output schema (story record, generation-time brief, prompt section, cast dossier, causal-pressure record, accepted-segment archive entry): name the schema, the consumers of that schema, and whether the extension is additive-only or breaking>
