...
3. **Cross-artifact boundary under audit.** Both routes consume `buildStoryRecordHygieneSnapshot` + `parseRecordHygieneResponse` (SPEC027RECHYGASS-005) and `compileRecordHygienePrompt` (SPEC027RECHYGASS-004). New-module reachability: both routes are registered in `server.ts` (mirroring the `registerIdeateRoutes` call at `:93`). `analyze` recompiles server-side and ignores any client-supplied prompt/subset/edit/write field.
