...
3. **Cross-artifact boundary under audit.** The builder produces `StoryRecordHygieneSnapshot`/`HygieneRecord`/`HygieneReferenceSummary` (types from SPEC027RECHYGASS-002), gated by `isHygieneActive` (002) and labelled/status-projected via core registry functions (`projectRecordStatus`, `registry.ts:63`). The parser validates against the citation-key set the compiler emits (SPEC027RECHYGASS-004). It mirrors the error-category style of `ideate-routes.ts` (`{ ok:false, kind:… }`).
