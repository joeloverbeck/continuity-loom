...
3. Shared boundary under audit: the rail consumes the section-fill helper + `isRequiredNow` from `archive/tickets/SPEC017GENBRIVIS-003.md` (chip data) and the section cards/heading ids from `archive/tickets/SPEC017GENBRIVIS-002.md` (jump targets). IntersectionObserver lives behind a small seam so the chip/anchor logic is unit-testable while scroll-spy is covered by the manual pass (SPEC017GENBRIVIS-006); jsdom cannot exercise IntersectionObserver directly.
