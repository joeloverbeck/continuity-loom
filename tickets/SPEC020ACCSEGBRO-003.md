...
Add a "Jump to latest" control (reusing the SPEC020ACCSEGBRO-002 scroll+focus-the-latest helper) and a "Back to top" control that moves focus to the top of the archive. Both move **focus**, not just the viewport, and carry accessible names. Render them only when useful (the list is taller than the viewport / the user has scrolled away) — e.g. floating/sticky.
...
1. `packages/web/src/accepted-segments/AcceptedSegmentsView.test.tsx` — add Expand-all / Collapse-all (assert prose presence/absence across all segments), Jump-to-latest (focus via `document.activeElement`), Back-to-top (focus target), and accessible-name cases. Reuse the `window.matchMedia` / `Element.prototype.scrollIntoView` stubs introduced in SPEC020ACCSEGBRO-002.
