...
1. `Assumption Reassessment (YYYY-MM-DD)`:
   ...
   - For tickets touching FOUNDATIONS-aligned enforcement surfaces (validation rules, fail-closed gates, the secret firewall, deterministic compilation, HARD-GATE semantics), restate the FOUNDATIONS principle under audit before trusting the spec narrative.
...
9. For tickets touching skill HARD-GATE semantics, fail-closed validation, or deterministic-compilation surfaces, verify the change does not weaken the secret firewall (FOUNDATIONS §15) or break deterministic compilation (§8).
10. For tickets extending an existing output schema (story record, generation-time brief, prompt section, cast dossier, causal-pressure record, accepted-segment archive entry), verify consumers of that schema have been updated, or the extension is additive-only (new optional field with a default).
