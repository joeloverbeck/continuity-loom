...
| `docs/specs/cast-possibilities-prompt-template.md` | Saved-moment Cast Possibilities source profile, shared cast rendering, strict card contract, provider boundary, and session-only scratch. | specification | domain authority for the Cast Possibilities prompt template |
...
- `docs/specs/cast-possibilities-prompt-template.md`
...
The root and workspace package versions are private-package metadata and may remain `0.0.0`. The implemented prompt template, compiler, and compiler-contract versions are separate contract versions whose source of truth is `packages/core/src/version.ts`; template is `1.12.0`, compiler is `1.14.0`, and compiler contract is `1.17.0`. The active `accepted-segment-change-review` post-acceptance assistance workflow declares its own template version `2.1.0` and compiler version `2.0.0`, contract version `2.1.0`, and the output identity `accepted_segment_change_review.v2` under `packages/core/src/compiler/change-review/`. The active `cast-possibilities` prose-aligned assistance workflow independently declares template and contract version `1.0.0`, compiler version `1.0.7`, and output identity `cast_possibilities.v1` under `packages/core/src/compiler/cast-possibilities/`.
