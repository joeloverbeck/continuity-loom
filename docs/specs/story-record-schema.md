...
type AcceptedSegmentProvenance =
  | {
      ...
      reasoningIntent: "minimal" | "low" | "medium" | "high" | "xhigh" | "max" | "provider_default";
      topP?: number;
      versions: {
        template: string;
        compiler: string;
...
- OpenRouter provenance records the exact declared prose-request intent. Its existing `maxOutputTokens` field is the actual Prose ceiling serialized in the accepted segment's finalized request; the settings split does not rename that field or add a provenance version. `reasoningIntent` is copied from the successful candidate's server-produced finalized-request metadata at explicit acceptance, never reconstructed from current Settings. New accepted OpenRouter prose uses exactly one canonical sent effort: `minimal`, `low`, `medium`, `high`, `xhigh`, or `max`. The historical-only `provider_default` marker means that an older accepted row predates exact effort capture; it does not identify or reconstruct a provider-side effort and is invalid for new acceptance or future request construction. Explicit temperature mode requires its sent numeric temperature; provider-default temperature mode forbids a numeric temperature because the provider's effective value is unknown. Top P appears only when sent. Reasoning content, summaries, details, and provider-reported reasoning-token counts never enter accepted provenance. Assistance never creates accepted provenance. User-supplied provenance records the versions associated with the inspected prompt and has no model, provider, temperature mode, temperature, maximum-output-token, reasoning-intent, or top-p fields.
...
- Project schema migration v5 to v6 adds `reasoningIntent: "provider_default"` only to valid older OpenRouter provenance without reasoning intent. It preserves already-current OpenRouter rows and all user-supplied rows byte-for-byte, runs transactionally and idempotently, and preserves prose, sampling intent, completion ceiling, versions, timestamps, sequence, and ordering. Malformed metadata or any row/manifest write failure rolls back every row, schema version, and manifest change and leaves the project unopened with `migration-failed`.
...
- `audience_visibility` on a FACT is author metadata about how openly the fact is treated; it is **not a reader-concealment control** and does not conceal the fact from the reader. A FACT renders only among the POV-accessible facts, so its `audience_visibility` never reaches the compiled audience-knowledge block and produces no per-fact concealment annotation — a `hidden` FACT compiles identically to an `explicit` one. To conceal a POV-known premise from the reader (dramatic irony), model it as a SECRET with `pov_access: knows` and `audience_visibility: hidden`: the SECRET record is the sole reader-concealment authority, and a hidden SECRET is what populates the compiled "Audience does not know" line. Wiring `FACT.audience_visibility` into compiled concealment would introduce a second reader-concealment authority and would require a deliberate `docs/principles/FOUNDATIONS.md` amendment first (see `docs/adr/0004-secret-is-sole-reader-concealment-authority.md`). A deterministic advisory validation warning (`fact-hidden-audience-visibility-not-concealment`) nudges the author toward the SECRET model when a `hard_canon` (`fact_kind`) or `critical` (`salience`) FACT is set to `audience_visibility: hidden`; it never gates Preview or Generate.
...
Each included record renders its full payload as escaped canonical JSON plus one complete payload-derived label, projected status, citation key, and reference summaries. The stored browse label does not enter the projection, ordering, or citation assignment. ENTITY and CAST MEMBER payloads are excluded; references to those records may resolve only to their complete payload-derived label, with no other payload field imported. Accepted prose, candidates, story notes, story configuration, generation-time brief fields, active-working-set membership as prose authority, timestamps, user-order values, and provider settings are not part of this projection.
...
listed in the compiled prompt, dossier citations to the eligible dossier-key
set, and context citations to the compiled non-dossier citation-key set. The
...
The authoritative placeholder mapping, section order, empty-state rendering rules, validation matrix, and blocker/warning taxonomy live in `docs/specs/compiler-contract.md`.

Schema/template synchronization rules:

- Every prompt placeholder must appear in `docs/specs/compiler-contract.md`.
- Every template section required by `docs/principles/FOUNDATIONS.md` must have a deterministic source and empty-state behavior.
