...
- OpenRouter provenance records the exact declared prose-request intent. Its existing `maxOutputTokens` field is the actual Prose ceiling serialized in the accepted segment's finalized request; the settings split does not rename that field or add a provenance version. Explicit mode requires its sent numeric temperature; provider-default mode forbids a numeric temperature because the provider's effective value is unknown. Top P appears only when sent. Assistance never creates accepted provenance. User-supplied provenance records the versions associated with the inspected prompt and has no model, provider, temperature mode, temperature, maximum-output-token, or top-p fields.
