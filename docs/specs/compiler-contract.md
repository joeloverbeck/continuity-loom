...
1. Versioned constants in `packages/core/src/compiler/change-review/`, governed by `docs/principles/FOUNDATIONS.md`, PRD #145, the final-activation amendment ratified through issue #146, issue #149, and this contract.
...
3. Exactly the nineteen saved-draft CURRENT AUTHORITATIVE STATE and IMMEDIATE HANDOFF paths registered in `docs/specs/story-record-schema.md`. Missing and blank values remain explicit; no prose-readiness normalization or other Generation Brief surface is imported.
...
6. Deterministic accepted-segment spans, brief/record citation keys, disclosure counts, prompt length and token estimate, versions, and prompt fingerprint.
...
This profile excludes every older accepted segment, segment range, candidate, regeneration, automatic prose-derived summary, author-private note, story configuration value, prompt archive, prior assistance output, provider memory, hidden UI state, archived record, schema catalog, lifecycle catalog, JSON Patch vocabulary, creation payload, full proposed canonical value, and model-selected destination.
...
The only accepted-prose exception is the active accepted-segment-change-review source profile in §2.3. It reads exactly one explicitly requested latest accepted segment for bounded advisory review. No prompt compiler may read any other accepted-prose source or persist a derived summary.

Generation-time fields may override story defaults for the current prose request, but they do not override hard canon, current authoritative state, physical continuity, POV/reveal locks, or governing provider/platform policy. Accepted-Segment Change Review output has no override effect at all.
...
- `<ideation_slots>` from the single assignment produced by `compileIdeationPrompt(snapshot, ideationRequest)`, including operator name, operator id, definition, slate shrink status, slot citation keys, and any dormant-record modifier instruction.
...
- `<ideation_output_format>` from a template constant defining the pure-JSON `grounded_ideation.v1` contract, exact operator-id copy rule, status discriminator, and malformed-output discard rule.
...
`compileIdeationPrompt(snapshot, ideationRequest)` is the sole Ideate compile seam. It calls `assignSlots` exactly once and derives `{ prompt, assignment, outputSchema, parseContext, metadata }` from that assignment. The request-specific provider-safe schema requires exactly `contract` and `ideas`; constrains the contract to `grounded_ideation.v1`; constrains slot numbers and operator ids to the compiled assignment; selects `headline` or `question` by mode; requires the always-present `status`, mode, `why`, and `grounds` fields; and constrains grounds items to snapshot citation keys. `status` is `idea` or `skipped`; skipped items carry empty mode and `why` strings plus an empty grounds array. Provider-unsupported conditional and cardinality keywords are omitted and the pure local parser re-enforces those rules fail-closed.

Ideation preview parses the complete request locally and returns prompt bytes plus fingerprint only when the existing ideation readiness gate and focus validation pass. Both preview and send build their strict finalized request from the same compile artifact and request-options helper: `response_format` is strict `json_schema`, provider fallback and transforms are disabled, and plugins/tools are empty. The fingerprint therefore includes the identical schema and request policy on both paths. Ideation send requires nonblank `expectedPromptFingerprint` and `expectedRequestFingerprint`; the server reparses the complete request, rebuilds current snapshot/readiness and the compile artifact, compares both fingerprints, and only then invokes OpenRouter once. A model without both `response_format` and `structured_outputs` is refused by capability admission before transport. Missing, malformed, over-limit, incompatible, or stale requests make zero provider calls; a valid mismatch returns the existing stale-prompt or stale-provider-request refusal. Client-supplied prompt text is never accepted.

After a normal decoded completion, the core parser accepts one complete JSON value and no alternate grammar. It requires the exact top-level envelope, every assigned slot exactly once, no unassigned or duplicate slot, the operator id assigned to each slot, the requested mode field, a valid status, and the status-specific empty/nonempty field rules. Any semantic failure quarantines the whole response with a typed content-free reason. Unknown citations remain a non-blocking flag after otherwise valid parsing. Rejected provider text never reaches the browser, and no parse or diagnostic action retries.
...
`<citation_legend>`, `<card_constraints>`, and `<output_instructions>` in that
order. Complete saved source precedes the procedure and strict schema. Target
regeneration keeps the same order and projects exactly one eligible character
plus its explicit three-summary avoid list.
...
### 4.2 Accepted-Segment Change Review source mapping

| Prompt destination | Deterministic source | Empty-state behavior | Validation and exclusion rule |
|---|---|---|---|
| `<accepted_segment_change_review_role>` | versioned constant | never empty | advisory accounting; no prose, future possibility, canonical value, patch, lifecycle action, or creation payload |
...
| `<accepted_segment_change_review_request>` | accepted-segment id/sequence/accepted timestamp, selected scope source count, versions | never empty | trusted source metadata is server-owned and never accepted from model output |
...
| `{active_cast_voice_pressure_pins}` | CAST MEMBER.voice_anchor + CURRENT CAST VOICE PRESSURE + CAST VOICE OVERRIDES + active working-set local_function | Optional prompt preference | Warn if absent when durable anchors are sufficient; block only if supplied pressure contradicts authority or required voice/body authority has no durable/compressed/current source | Omit or `None` if no active person-like cast | Salience duplicate; includes dialogue/narration/nonverbal/silence scopes. Each pin leads with the CAST MEMBER's validated linked ENTITY primary display name; the CAST MEMBER `identity.one_line` follows as retained descriptive content, never as the identity label, and no raw stored identifier stands in for the name. Present-minor current pressure renders in `{present_minor_cast_notes}` for prose prompts. |
...
- Accepted-Segment Change Review always renders all nineteen review paths. Missing values render an explicit missing state and blank values remain explicit; neither is omitted. An empty `items` list is valid only with all six unique reasoned coverage rows and remains visibly unverified advice.
- `<record_contrast_records>` renders `No non-archived records exist in the selected record scope.` when no full records qualify. Reference stubs, if any, render in their declared sub-block. Empty records do not disable send because change-review items and coverage may still be useful.
- An absent accepted segment is not an empty state. It blocks the operation with `no-accepted-segment`.
...
Any change to Ideate source, request shape, assignment, section order, output schema, parser, provider policy, quarantine, regeneration, or session scratch must update `docs/specs/ideation-prompt-template.md`, this contract, public core types, the golden Ideate prompt, and focused core/server/browser tests in the same revision. Widening prompt authority or output use also requires a FOUNDATIONS amendment approved under §1.1.
