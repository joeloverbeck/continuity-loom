...
| `{pov_cannot_perceive_now}` | POV KNOWLEDGE PROFILE.pov_cannot_perceive_now + visibility/line-of-sight state | Required when perception limits matter | Block if ambiguous perception or hidden plan depends on it and absent | Omit line when empty | Prevents impossible perception. |
