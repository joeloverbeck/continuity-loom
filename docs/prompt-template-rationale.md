...
Every cast lane leads with the character's name. The dossier heading, each active voice-pressure pin, and each present-minor and offstage-relevance entry begin with the CAST MEMBER's validated linked ENTITY primary display name, resolved from that ENTITY record within the validation snapshot. The descriptive `identity.one_line` is retained beside the name as content, but it is not the identity label. Two reasons drive this. First, an author auditing whether a voice pressure landed on the intended character should read a name, not infer identity from a description. Second, and more seriously, two cast records with similar one-line descriptions become mutually indistinguishable to the external writer when the lane leads with the description; leading with the name is what keeps distinct voices attributable to distinct people. Because the compiler may read only the validation snapshot, a name it cannot resolve there is a blocking condition rather than a degraded rendering: the linked ENTITY reference is required in every cast band, and a stored record identifier is never substituted for a name. This is a naming and reference-integrity rule; membership, cast bands, deterministic ordering (which still derives from the CAST MEMBER's own display label), and every rendered pressure and dossier field are unchanged. (PRD #129; #125, #127; see `docs/specs/compiler-contract.md` §9.)
