...
- Feed is correct (selected `FACT` with `fact_kind=hard_canon`); empty phrase is contract-specified (`compiler-contract.md:97`).
- **Constitutional conflict**: omitting a universal section needs an amendment (`FOUNDATIONS.md:910`, §9 list `:274`). Only cast-band sections are sanctioned-omittable.
...
- Pure rendering bug; template implies a single line (`prompt-template.md:90,94`); no contract change.
...
- Feeds correct and surfaced (compiled from FACT/BELIEF/SECRET keyed to POV). Omitting empty *lines within* a section is the sanctioned pattern (`compile-prompt.ts:128-134`, `compiler-contract.md:261-265`); section tag + static rule text always remain, so §29.4 is satisfied — no FOUNDATIONS amendment.
- Coverage-widening: applies uniformly to all four POV value-lines, not just `pov_does_not_know`.
- Disposition: **ACTION — ticket `POVEMPTYLINE-001`** (compiler + `compiler-contract.md` §4/§8 + `prompt-template.md` in lockstep).
...
- No dedicated field exists (`generation-brief.ts:30-48`); field-guidance maps the field only to `{line_of_sight_and_visibility}` (`field-guidance-brief-config.ts:236-238`); contract names an unimplemented profile source (`compiler-contract.md:123`).
...
- Each compiler change carries its in-same-revision `docs/compiler-contract.md` update (§10 change-control) and golden-test refresh.
