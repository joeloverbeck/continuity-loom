...
- The OPEN THREAD schema (`causal-pressure.ts:128-141`) matches `docs/story-record-schema.md` §8.7 field-for-field — no schema defect; no validation change.
...
- No §29 hard-fail tripped; no FOUNDATIONS amendment required. An optional one-line `docs/story-record-schema.md` note (per OPENTHREAD-002 / EVENTSEQ-001) keeps the schema doc consistent with the corrected guidance.
